...
export default class App {
  ...
  async setupThreeAsync() {
    ...
    const initialGeometryVertexLength =
      universe.get_initial_geometry_vertex_length();
  ...
  onRender(time: DOMHighResTimeStamp, frame: XRFrame) {
    ...
    if (this.modes.isBall) {
      ...
      for (let ball of balls) {
        ball.mesh.position.add(
          ball.velocity.clone().multiplyScalar(deltaTime / 1000)
        );
    ...
    universe.update(
      time * 0.001,
      V3F.new(camera.position.x, camera.position.y, camera.position.z)
    );
    universe.draw();
