...
export function updateBufferGeometry(
  ...
  {
    const positionListPointer =
      universe.get_geometry_buffer_position_list_ptr(i);
  ...
  }
  {
    ...
    const normalList = new Float32Array(
      memory.buffer,
      normalListPointer,
      vertex_length * 3
    );
  ...
  }
  {
    ...
    const colorList = new Float32Array(
      memory.buffer,
      colorListPointer,
      vertex_length * 3
    );
